...
public:
	...
	bool initialize(Camera * aCamera,unsigned int aShadowMapRes)
	{
		...
		/* 加载各个着色器 */
		standardShader = new Shader("src/Shaders/standard.vert", "src/Shaders/standard.frag");//最后输出结果用的
		voxelizationShader = new Shader("src/Shaders/voxelization.vert", "src/Shaders/voxelization.frag", "src/Shaders/voxelization.geom");//体素化用的
		shadowShader = new Shader("src/Shaders/shadow.vert", "src/Shaders/shadow.frag");//生成深度纹理用的
